...
<div class="font-Poppins bg-[#050634f2] selection:text-secondary selection:bg-darkblue">
    ...
    @foreach($data as $item)
            <div class="max-w-sm border border-secondary rounded-lg shadow-md bg-accent my-3">
                <a href="/itmatsuri/tiket/detail?id={{$item->id}}">
                    <img class="rounded-t-lg" src="{{ asset('assets/'.$item->gambar_tiket) }}" alt="" />
                </a>
                <div class="p-5">
                    <a href="/itmatsuri/tiket/detail?id={{$item->id}}">
                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-slate-100 uppercase">{{$item->nama_tiket}}</h5>
                    </a>
                    <p class="mb-3 font-normal text-gray-900">{{$item->harga_tiket}}</p>
                    <p class="mb-3 font-normal text-gray-700">{{$item->desc_tiket}}</p>
                    <a href="/itmatsuri/tiket/detail?id={{$item->id}}" class="inline-flex bg-darkblue items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        Pilih TIket
                    </a>
                </div>
            </div>
