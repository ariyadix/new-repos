...
<body>
	<center>
		<h2>Grafik Identitas Kelompok</h2>
		...
		<tbody>
			...
			?>
				<tr>
					...
					<td><?php echo $d['status_kelompok']; ?></td>
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT tahun_berdiri FROM tb_identitas_kelompok order by tahun_berdiri ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['tahun_berdiri'];
						echo "\"";
						echo $d['tahun_berdiri'];
						echo "\",";
						$no++;
					}
					?>
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_identitas_kelompok where tahun_berdiri=" . "'" . $data_tahun[$i] . "'");
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_status = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT status_kelompok FROM tb_identitas_kelompok order by status_kelompok ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_status[$no] = $d['status_kelompok'];
						echo "\"";
						echo $d['status_kelompok'];
						echo "\",";
						$no++;
					}
					?>
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_status)) {
							$tahun = mysqli_query($koneksi, "select * from tb_identitas_kelompok where status_kelompok=" . "'" . $data_status[$i] . "'");
							echo mysqli_num_rows($tahun);
							$i++;
						?>,
						<?php } ?>
