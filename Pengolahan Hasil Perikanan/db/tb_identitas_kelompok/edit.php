<?php
include('../koneksi.php');
$data = mysqli_query($koneksi, "select*from tb_identitas_kelompok where Id=" . "'" . $_GET['id'] . "'");
?>
<!DOCTYPE html>
<html>
...
<body id="page-top" bgcolor="red">
	<div class="content container mt-4 mb-4">
		...
		?>
			<form method="post" name="formKelompok" action="edit_aksi.php" enctype="multipart/form-data" onsubmit="return validateForm()">
				<div class="form-group">
					<input type="hidden" name="id" value="<?php echo $_GET['id'] ?>">
					<label>Nama Kelompok : <br></label>
					<input value="<?php echo $d['nama_kelompok']; ?>" type="text" name="nama_kelompok" class="form-control" placeholder="Masukan Nama Kelompok" required />
				</div>
				<div class="form-group">
					<label>Tahun Berdiri : <br></label>
					<input value="<?php echo $d['tahun_berdiri']; ?>" type="number" min="0" max="9999" pattern="[0-9]{4}" name="tahun_berdiri" class="form-control" placeholder="Masukan Tahun Berdiri" required />
				</div>
				<div class="form-group">
					<label>Nomor Surat : <br></label>
					<input value="<?php echo $d['nomor_surat']; ?>" type="text" name="nomor_surat" class="form-control" placeholder="Masukan Nomor Surat" required />
				</div>
				<div class="form-group">
					<label>Upload Surat :</label><br>
					<a href="../file/surat/<?php echo $d['upload_surat']; ?>" target=”_blank”>Lihat Surat</a>
					<br><small style="align: left; color:grey;">Ganti File Surat</small>
					<input type="file" name="upload_surat" />
				</div>
				<div class="form-group">
					<label>Status Kelompok : <br></label>
					<select name="status_kelompok" id="status_kelompok" class="form-control" placeholder="Masukkan Status Kelompok" required>
						<option <?php if (!empty($d['status_kelompok'])) {
									echo $d['status_kelompok'] == '0' ? 'selected' : '';
								} ?> value="0">Pilih Status Kelompok</option>
						<option <?php if (!empty($d['status_kelompok'])) {
									echo $d['status_kelompok'] == 'Pemula' ? 'selected' : '';
								} ?> value="Pemula">Pemula</option>
						<option <?php if (!empty($d['status_kelompok'])) {
									echo $d['status_kelompok'] == 'Utama' ? 'selected' : '';
								} ?> value="Utama">Utama</option>
					</select>
				</div>
				<div class="form-group">
					<label>Titik Lokasi : <br></label>
					<input value="<?php echo $d['titik_lokasi']; ?>" type="text" name="titik_lokasi" class="form-control" placeholder="Masukkan Titik Lokasi" required />
				</div>
				<div class="form-group">
					<label>Alamat : <br></label>
					<input value="<?php echo $d['alamat']; ?>" type="text" name="alamat" class="form-control" placeholder="Masukkan Alamat" required />
				</div>
				<div class="form-group">
					<label>Penyuluh Pedamping : <br></label>
					<input value="<?php echo $d['penyuluh_pendamping']; ?>" type="text" name="penyuluh_pendamping" class="form-control" placeholder="Masukkan Penyuluh Pendamping" required />
				</div>
				<div class="form-group">
					<label>Logo Kelompok : </label><br>
					<img src="../file/logo/<?php echo $d['logo_kelompok']; ?>" width="100px" height="100px" />
					<br><small style="align: left; color:grey;">Ganti Foto Logo</small>
					<input type="file" name="logo_kelompok" id="logo_kelompok" accept="image/jpg, image/jpeg, image/png, image/gif" onchange="return validasiFile()" />
				</div>
