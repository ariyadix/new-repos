...
<center>
	...
	<!-- <table border="1">
		...
		<tbody>
			<?php
			$no = 1;
			$data = mysqli_query($koneksi, "select * from tb_qry_trx_surat");
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT keterangan FROM tb_qry_trx_surat order by keterangan ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['keterangan'];
						echo "\"";
						echo $d['keterangan'];
						echo "\",";
						$no++;
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_qry_trx_surat where keterangan=" . "'" . $data_tahun[$i] . "'");
