...
<body>
	<center>
		<h2>Grafik Data Rencana Transaksi</h2>
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT volume FROM tb_qry_trx_rencana order by volume ASC");
					while ($d = mysqli_fetch_array($data)) {
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_qry_trx_rencana  where volume=" . "'" . $data_tahun[$i] . "'");
