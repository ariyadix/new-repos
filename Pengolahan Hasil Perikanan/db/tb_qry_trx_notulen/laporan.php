...
<center>
	...
	<!-- <table border="1">
		...
		<tbody>
			<?php
			$no = 1;
			$data = mysqli_query($koneksi, "select * from tb_qry_trx_notulen");
			while ($d = mysqli_fetch_array($data)) {
			?>
				<tr>
					<td><?php echo $no++; ?></td>
					<td><?php echo $d['jumlah_hadir']; ?></td>
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT jumlah_hadir FROM tb_qry_trx_notulen order by jumlah_hadir ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['jumlah_hadir'];
						echo "\"";
						echo $d['jumlah_hadir'];
						echo "\",";
						$no++;
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_qry_trx_notulen where jumlah_hadir=" . "'" . $data_tahun[$i] . "'");
