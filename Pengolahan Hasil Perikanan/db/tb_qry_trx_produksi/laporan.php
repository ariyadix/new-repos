...
<body>
	...
	<!-- <table border="1">
		...
		<tbody>
			<?php
			$no = 1;
			$data = mysqli_query($koneksi, "select * from tb_qry_trx_produksi");
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT volume FROM tb_qry_trx_produksi order by volume ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['volume'];
						echo "\"";
						echo $d['volume'];
						echo "\",";
						$no++;
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_qry_trx_produksi  where volume=" . "'" . $data_tahun[$i] . "'");
