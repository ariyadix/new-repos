...
<body>
	<center>
		<h2>Grafik Sarana Dan Prasarana</h2>
	...
	<!-- <table border="1">
		...
		<tbody>
			<?php
			$no = 1;
			$data = mysqli_query($koneksi, "select * from tb_sapras");
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT jenis FROM tb_sapras order by jenis ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['jenis'];
						echo "\"";
						echo $d['jenis'];
						echo "\",";
						$no++;
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_sapras where jenis=" . "'" . $data_tahun[$i] . "'");
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_status = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT keterangan FROM tb_sapras order by keterangan ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_status[$no] = $d['keterangan'];
						echo "\"";
						echo $d['keterangan'];
						echo "\",";
						$no++;
				...
				datasets: [{
					...
					data: [
						<?php
						$i = 0;
						while ($i < count($data_status)) {
							$tahun = mysqli_query($koneksi, "select * from tb_sapras where keterangan=" . "'" . $data_status[$i] . "'");
							echo mysqli_num_rows($tahun);
							$i++;
						?>,
						<?php } ?>
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_barang = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT kondisi_barang FROM tb_sapras order by kondisi_barang ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_barang[$no] = $d['kondisi_barang'];
						echo "\"";
						echo $d['kondisi_barang'];
						echo "\",";
						$no++;
					}
					?>
				...
				datasets: [{
					...
					data: [
						<?php
						$i = 0;
						while ($i < count($data_barang)) {
							$tahun = mysqli_query($koneksi, "select * from tb_sapras where kondisi_barang=" . "'" . $data_barang[$i] . "'");
							echo mysqli_num_rows($tahun);
							$i++;
						?>,
						<?php } ?>
