...
<center>
	...
	<!-- <table border="1">
		...
		<tbody>
			<?php
			$no = 1;
			$data = mysqli_query($koneksi, "select * from tb_bukutamu");
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT jabatan FROM tb_bukutamu order by jabatan ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['jabatan'];
						echo "\"";
						echo $d['jabatan'];
						echo "\",";
						$no++;
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_bukutamu where jabatan=" . "'" . $data_tahun[$i] . "'");
