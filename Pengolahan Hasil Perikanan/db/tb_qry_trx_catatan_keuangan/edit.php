<?php
include('../koneksi.php');
$data = mysqli_query($koneksi, "select*from tb_qry_trx_catatan_keuangan where Id=" . "'" . $_GET['id'] . "'");
?>
<!DOCTYPE html>
<html>
...
<body id="page-top" bgcolor="red">
	<div class="content container mt-4 mb-4">
		<h2>Edit Data Transaksi Keuangan</h2>
		<?php
		...
		?>
			<form method="post" name="formCttKeuangan" onsubmit="return validateForm()" action="edit_aksi.php" enctype="multipart/form-data">
				...
				<div class="form-group">
					...
					<input type="text" value="<?php echo $d['uraian']; ?>" name="uraian" class="form-control" placeholder="Masukan Uraian" required />
				</div>
				<div class="form-group">
					<label>Jenis Transaksi : <br></label>
					<select name="jenis_transaksi" class="form-control" placeholder="Masukkan Jenis Transaksi" required>
						<option <?php if (!empty($d['jenis_transaksi'])) {
									echo $d['jenis_transaksi'] == '0' ? 'selected' : '';
								} ?> value="0">Pilih Jenis Transaksi</option>
						<option <?php if (!empty($d['jenis_transaksi'])) {
									echo $d['jenis_transaksi'] == 'Masuk' ? 'selected' : '';
								} ?> value="Masuk">Masuk</option>
						<option <?php if (!empty($d['jenis_transaksi'])) {
									echo $d['jenis_transaksi'] == 'Keluar' ? 'selected' : '';
								} ?> value="Keluar">Keluar</option>
					</select>
				</div>
				<div class="form-group">
					<label>Kategori : <br></label>
					<select name="kategori" class="form-control" placeholder="Masukkan Kategori" required>
						<option <?php if (!empty($d['kategori'])) {
									echo $d['kategori'] == '0' ? 'selected' : '';
								} ?> value="0">Pilih Kategori</option>
						<option <?php if (!empty($d['kategori'])) {
									echo $d['kategori'] == 'Sumbangan' ? 'selected' : '';
								} ?> value="Sumbangan">Sumbangan</option>
						<option <?php if (!empty($d['kategori'])) {
									echo $d['kategori'] == 'Bantuan' ? 'selected' : '';
								} ?> value="Bantuan">Bantuan</option>
						<option <?php if (!empty($d['kategori'])) {
									echo $d['kategori'] == 'Iuran' ? 'selected' : '';
								} ?> value="Iuran">Iuran</option>
						<option <?php if (!empty($d['kategori'])) {
									echo $d['kategori'] == 'Tabungan' ? 'selected' : '';
								} ?> value="Tabungan">Tabungan</option>
						<option <?php if (!empty($d['kategori'])) {
									echo $d['kategori'] == 'Kas Kelompok' ? 'selected' : '';
								} ?> value="Kas Kelompok">Kas Kelompok</option>
					</select>
				</div>
				<div class="form-group">
