...
<center>
	...
	<!-- <table border="1">
		...
		<tbody>
			<?php
			$no = 1;
			$data = mysqli_query($koneksi, "select * from tb_qry_trx_catatan_keuangan");
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT jenis_transaksi FROM tb_qry_trx_catatan_keuangan order by jenis_transaksi ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['jenis_transaksi'];
						echo "\"";
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_qry_trx_catatan_keuangan where jenis_transaksi=" . "'" . $data_tahun[$i] . "'");
