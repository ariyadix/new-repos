		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT nama_ikan FROM tb_ikan order by nama_ikan ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['nama_ikan'];
						echo "\"";
						echo $d['nama_ikan'];
						echo "\",";
						$no++;
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_ikan  where nama_ikan=" . "'" . $data_tahun[$i] . "'");
