...
<body>
	<center>
		<h2>Grafik Data Kemasan Produk</h2>
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT ukuran_kemasan FROM tb_kemasan_produk order by ukuran_kemasan ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['ukuran_kemasan'];
						echo "\"";
						echo $d['ukuran_kemasan'];
						echo "\",";
						$no++;
					}
					?>
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_kemasan_produk  where ukuran_kemasan=" . "'" . $data_tahun[$i] . "'");
