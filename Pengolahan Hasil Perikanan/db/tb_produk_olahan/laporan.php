...
<body>
	<center>
		<h2>Grafik Data Produk Olahan</h2>
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT id_ikan FROM tb_produk_olahan order by id_ikan ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['id_ikan'];
						echo "\"";
						echo $d['id_ikan'];
						echo "\",";
						$no++;
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_produk_olahan  where id_ikan=" . "'" . $data_tahun[$i] . "'");
