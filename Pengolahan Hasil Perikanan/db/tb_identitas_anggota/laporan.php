...
<body>
<center>
		<h2>Grafik Identitas Anggota</h2>
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_tahun = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT umur FROM tb_identitas_anggota order by umur ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_tahun[$no] = $d['umur'];
						echo "\"";
						echo $d['umur'];
						echo "\",";
						$no++;
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_tahun)) {
							$tahun = mysqli_query($koneksi, "select * from tb_identitas_anggota where umur=" . "'" . $data_tahun[$i] . "'");
		...
		var myChart = new Chart(ctx, {
			...
			data: {
				labels: [
					...
					$no = 0;
					$data_status = [];
					$data = mysqli_query($koneksi, "SELECT DISTINCT tingkat_pendidikan FROM tb_identitas_anggota order by tingkat_pendidikan ASC");
					while ($d = mysqli_fetch_array($data)) {
						$data_status[$no] = $d['tingkat_pendidikan'];
						echo "\"";
						echo $d['tingkat_pendidikan'];
						echo "\",";
						$no++;
					}
					?>
				...
				datasets: [{
					...
					data: [
						...
						while ($i < count($data_status)) {
							$tahun = mysqli_query($koneksi, "select * from tb_identitas_anggota where tingkat_pendidikan=" . "'" . $data_status[$i] . "'");
							echo mysqli_num_rows($tahun);
							$i++;
						?>,
						<?php } ?>
