<?php
include('../koneksi.php');
$data = mysqli_query($koneksi, "select*from tb_identitas_anggota where Id=" . "'" . $_GET['id'] . "'");
?>
<!DOCTYPE html>
<html>
...
<body id="page-top" bgcolor="red">
	<div class="content container mt-4 mb-4">
		<h2>Edit Data Identitas Anggota</h2>
		<?php
		while ($d = mysqli_fetch_array($data)) {
		?>
			<form method="post" action="edit_aksi.php" name="formAnggota" enctype="multipart/form-data" onsubmit="return validateForm()">
				...
				<div class="form-group">
					<label>Nama : <br></label>
					<input type="text" value="<?php echo $d['nama']; ?>" name="nama" class="form-control" placeholder="Masukan Nama" required />
				</div>
				<div class="form-group">
					<label>NIK : <br></label>
					<input type="number" min="0" max="9999999999999999" pattern="[0-9]{16}" value="<?php echo $d['nik']; ?>" name="nik" class="form-control" placeholder="Masukan NIK" required />
				</div>
				<div class="form-group">
					<label>Jabatan Kelompok : <br></label>
					<select name="jabatan_kelompok" class="form-control" placeholder="Masukkan Jabatan Kelompok" required>
						<option <?php if (!empty($d['jabatan_kelompok'])) {
									echo $d['jabatan_kelompok'] == '0' ? 'selected' : '';
								} ?> value="0">Pilih Jabatan Kelompok</option>
						<option <?php if (!empty($d['jabatan_kelompok'])) {
									echo $d['jabatan_kelompok'] == 'Ketua Kelompok' ? 'selected' : '';
								} ?> value="Ketua Kelompok">Ketua Kelompok</option>
						<option <?php if (!empty($d['jabatan_kelompok'])) {
									echo $d['jabatan_kelompok'] == 'Anggota Kelompok' ? 'selected' : '';
								} ?> value="Anggota Kelompok">Anggota Kelompok</option>
					</select>
				</div>
				<div class="form-group">
					<label>Alamat : <br></label>
					<input type="text" value="<?php echo $d['alamat']; ?>" name="alamat" class="form-control" placeholder="Masukkan Alamat" required />
				</div>
				<div class="form-group">
					<label>No HP : <br></label>
					<input type="tel" value="<?php echo $d['hp']; ?>" name="hp" class="form-control" placeholder="Masukkan No HP" required />
				</div>
				<div class="form-group">
					<label>Tingkat Pendidikan : <br></label>
					<select name="tingkat_pendidikan" class="form-control" placeholder="Masukkan Tingkat Pendidikan" required>
						<option <?php if (!empty($d['tingkat_pendidikan'])) {
									echo $d['tingkat_pendidikan'] == '0' ? 'selected' : '';
								} ?> value="0">Pilih Tingkat Pendidikan</option>
						<option <?php if (!empty($d['tingkat_pendidikan'])) {
									echo $d['tingkat_pendidikan'] == 'Tidak lulus SD' ? 'selected' : '';
								} ?> value="Tidak lulus SD">Tidak lulus SD</option>
						<option <?php if (!empty($d['tingkat_pendidikan'])) {
									echo $d['tingkat_pendidikan'] == 'SD/MI' ? 'selected' : '';
								} ?> value="SD/MI">SD/MI</option>
						<option <?php if (!empty($d['tingkat_pendidikan'])) {
									echo $d['tingkat_pendidikan'] == 'SMP/SLTP/MTs' ? 'selected' : '';
								} ?> value="SMP/SLTP/MTs">SMP/SLTP/MTs</option>
						<option <?php if (!empty($d['tingkat_pendidikan'])) {
									echo $d['tingkat_pendidikan'] == 'SMA/SLTA/MA/SMK' ? 'selected' : '';
								} ?> value="SMA/SLTA/MA/SMK">SMA/SLTA/MA/SMK</option>
						<option <?php if (!empty($d['tingkat_pendidikan'])) {
									echo $d['tingkat_pendidikan'] == 'D3' ? 'selected' : '';
								} ?> value="D3">D3</option>
						<option <?php if (!empty($d['tingkat_pendidikan'])) {
									echo $d['tingkat_pendidikan'] == 'D4' ? 'selected' : '';
								} ?> value="D4">D4</option>
						<option <?php if (!empty($d['tingkat_pendidikan'])) {
									echo $d['tingkat_pendidikan'] == 'S1' ? 'selected' : '';
								} ?> value="S1">S1</option>
						<option <?php if (!empty($d['tingkat_pendidikan'])) {
									echo $d['tingkat_pendidikan'] == 'S2' ? 'selected' : '';
								} ?> value="S2">S2</option>
					</select>
				</div>
				<div class="form-group">
					<label>Umur : <br></label>
					<input type="number" value="<?php echo $d['umur']; ?>" name="umur" class="form-control" placeholder="Masukkan Umur" required />
				</div>
				<div class="form-group">
					<label>Usaha Selain Perikanan : <br></label>
					<input type="text" value="<?php echo $d['usaha_selain_perikanan']; ?>" name="usaha_selain_perikanan" class="form-control" placeholder="Masukkan Usaha Selain Perikanan" required />
				</div>
				<div class="form-group">
					<label>Mulai Usaha Perikanan : <br></label>
					<input type="number" min="0" max="9999" pattern="[0-9]{4}" value="<?php echo $d['mulai_usaha_perikanan']; ?>" name="mulai_usaha_perikanan" class="form-control" placeholder="Masukkan Mulai Usaha Perikanan" required />
				</div>
				<div class="form-group">
					<label>Foto : </label><br>
					<img src="../file/photo/<?php echo $d['photo']; ?>" width="100px" height="100px" />
					<br><small style="align: left; color:grey;">Ganti Foto</small>
...
<script>
	var gradeField = document.querySelector("input[name=mulai_usaha_perikanan]");
	...
	function validateForm() {
		if (document.forms["formAnggota"]["jabatan_kelompok"].selectedIndex == 0) {
			alert("Pilih Jabatan Kelompok!");
			document.forms["formAnggota"]["jabatan_kelompok"].focus();
			return false;
		}
		if (document.forms["formAnggota"]["tingkat_pendidikan"].selectedIndex == 0) {
			alert("Pilih Tingkat Pendidikan!");
			document.forms["formAnggota"]["tingkat_pendidikan"].focus();
			return false;
		}
	}
</script>
